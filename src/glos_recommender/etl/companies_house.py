...
# Find the current Companies House BasicCompanyData zip URL.
...
# Download the Companies House bulk zip to data/raw.
...
# Numeric size proxy from Companies House accounts category.
...
# Map SIC text to MatchKite sector tags.
...
# Title-case a company name for seed rows.
...
# Load GL/BS active companies from the CH zip.
...
# Rank local CH employers by size proxy.
def rank_gl_employers(
    ...
    *,
    ...
    # real employer homepage is supplied manually (no commercial-site crawling).
    df["website"] = ""
...
# Convert ranked CH rows into companies_seed-shaped rows.
...
# Load curated public-sector employer anchors CSV.
...
# Merge ranked/anchor rows into companies_seed.csv.
def upsert_into_seed(
    ...
    for j, (_, row) in enumerate(to_add.iterrows()):
        ...
        if not str(d.get("source") or "").strip():
            d["source"] = "companies_house"
        # Keep only seed columns (now includes source)
